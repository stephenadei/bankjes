...
def test_google_gone_entirely():
    ...
    self-hosted (fase 4)."""
    # Pin actual endpoints (URLs/hosts), not prose — comments may explain
    # the removal in words.
    for page in ("index.html", "onderzoek.html", "admin.html", "coming-soon.html"):
        html = _read(page).lower()
        for host in ("google.com", "googleapis.com", "gstatic.com"):
            assert host not in html, f"{host} left in {page}"
